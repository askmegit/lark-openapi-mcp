        ...
        data: zod_1.z.object({
            ...
            node_list: zod_1.z
                ...
                approver_chosen_range: zod_1.z
                    ...
                    id_list: zod_1.z
                        .array(zod_1.z.string())
                        .describe('ID list.- When type is set to ALL, no value needs to be provided.- When type is set to PERSONAL, provide the user ID, and the ID type must be consistent with the value of user_id_type.- When type is set to ROLE, provide the role ID. Obtaining method: After successfully [creating a role], you can obtain the role ID from the return result')
                        .optional(),
                }))
            ...
            config: zod_1.z
                ...
                .describe('Approval definition configuration item, used to configure whether the corresponding approval definition can be modified by the user in the [Approval Admin]')
                .optional(),
            icon: zod_1.z
                .number()
...
exports.approvalV4ApprovalUnsubscribe = {
    ...
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({ approval_code: zod_1.z.string().describe('Review Definition Unique Identifier').optional() }).optional(),
    },
};
        ...
        data: zod_1.z.object({
            ...
            external: zod_1.z
                ...
                action_callback_key: zod_1.z
                    .string()
                    .describe('Request parameter encryption key. If this parameter is configured, the request parameters will be encrypted and the request needs to be decrypted after receiving the request. For the encryption and decryption algorithm, refer to [Related External Option Description]')
                    .optional(),
...
exports.approvalV4ExternalInstanceCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            user_id: zod_1.z
                .string()
                .describe('Approval Initiator user_id. The initiator can see all initiated approvals in the "Initiated" list in the Approval Center. In the "Pending", "Completed", and "CC" lists, this field is used to display the initiator of the approval. For the method to obtain the user ID, see [How to Obtain User ID].**Note**: At least one of the initiator\'s `open_id` or `user_id` must be provided')
                .optional(),
            user_name: zod_1.z
                .string()
            ...
            open_id: zod_1.z
                .string()
                .describe('Approval Initiator open_id. The initiator can see all initiated approvals in the "Initiated" list in the Approval Center. In the "Pending", "Completed", and "CC" lists, this field is used to display the initiator of the approval. For the method to obtain the open ID, see [How to Obtain Open ID].**Note**: At least one of the initiator\'s `open_id` or `user_id` must be provided')
                .optional(),
            department_id: zod_1.z
                .string()
                .describe('Department ID of the initiator, which is used to display the department of the initiator in the Approval Center list. If the value is not provided, it will not be displayed. For the method to obtain the department ID, see [Department ID].**Note**: If the user has not joined any department, pass an empty string `""`, and it will default to displaying the company name. If the `department_name` parameter is provided, the corresponding department name will be displayed')
                .optional(),
            ...
            task_list: zod_1.z
                ...
                user_id: zod_1.z
                    .string()
                    .describe("Approver's user_id, the method to obtain it can be found in [How to Obtain User ID].**Notes**:- This task will appear in the **Pending** or **Completed** list in the approver's Feishu Approval Center.- At least one of `user_id` or `open_id` must be provided")
                    .optional(),
                open_id: zod_1.z
                    .string()
                    .describe("Approver's open_id, the method to obtain it can be found in [How to Obtain Open ID].**Notes**:- This task will appear in the **Pending** or **Completed** list in the approver's Feishu Approval Center.- At least one of `user_id` or `open_id` must be provided")
                    .optional(),
                title: zod_1.z
                    .string()
                ...
                update_time: zod_1.z
                    .string()
                    .describe('The last update time of the task, unix millisecond timestamp, which is used to push data versioning; update the policy update_time in the same instance')
                    .optional(),
                ...
                node_name: zod_1.z
                    ...
                    .optional(),
                generate_type: zod_1.z
                    .enum(['EXTERNAL_CONSIGN', 'DEFAULT'])
                    .describe('task generation type, can be left blank, but do not fill in the blank string. Options:EXTERNAL_CONSIGN(ExternalConsign Give the agent a completed task),DEFAULT(Default task generated by system)')
                    .optional(),
            }))
                .describe('Task list')
                .optional(),
            cc_list: zod_1.z
                ...
                user_id: zod_1.z
                    .string()
                    .describe("CC recipient's `user_id`. For the method of obtaining, please refer to [How to get a user's User ID].**Note**: At least one of the CC recipient's `open_id` and `user_id` must be provided")
                    .optional(),
                open_id: zod_1.z
                    .string()
                    .describe("CC recipient's `open_id`. For the method of obtaining, please refer to [How to get a user's Open ID].**Note**: At least one of the CC recipient's `open_id` and `user_id` must be provided")
                    .optional(),
                links: zod_1.z
                    .object({
                    pc_link: zod_1.z
                ...
                read_status: zod_1.z.enum(['READ', 'UNREAD']).describe('Read status Options:READ(read),UNREAD(unread)'),
                extra: zod_1.z
                    .string()
                ...
                update_time: zod_1.z
                    .string()
                    .describe('CC latest update time, unix millisecond timestamp, used for pushing data versions. If the `update_mode` value is `UPDATE`, the approval instance information in the approval center will only be updated when the transmitted `update_time` changes (increases)'),
                display_method: zod_1.z
                    .enum(['BROWSER', 'SIDEBAR', 'NORMAL', 'TRUSTEESHIP'])
            ...
            trusteeship_cache_config: zod_1.z
                ...
                .optional(),
            resource_region: zod_1.z
                .string()
                .describe('The area where the resource is located, the field for internal statistics does not need to be filled in')
                .optional(),
        }),
    },
};
    ...
    schema: {
        data: zod_1.z
            ...
            parent_comment_id: zod_1.z
                .string()
                .describe('Parent comment ID, if it is a reply comment, this value needs to be passed in. The method to obtain it:- Successfully calling the current interface will return the ID of this comment, you can save it for next use.- Call the [Get Comment] interface to get the comment ID')
                .optional(),
            comment_id: zod_1.z
                .string()
                .describe('Comment ID. If you need to edit or delete a comment, you need to pass the ID of that comment to the current parameter. The method to obtain it:- Successfully calling the current interface will return the ID of this comment, you can save it for next use.- Call the [Get Comment] interface to get the comment ID')
                .optional(),
            disable_bot: zod_1.z
                .boolean()
...
exports.approvalV4InstanceCommentDelete = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            instance_id: zod_1.z
                .string()
                .describe('Approval instance Code. The methods to obtain it are as follows:- [Create an approval instance] and retrieve the approval instance Code from the returned result.- Call [Batch get approval instance IDs] to obtain the approval instance Code within the specified approval definition.- Call [Query instance list] and set filter conditions to query the specified approval instance Code.**Note**: Custom approval instance IDs are supported'),
            comment_id: zod_1.z
                .string()
                .describe('Comment ID. The methods to obtain it are as follows:- Call [Create a comment], and the comment ID will be returned upon successful creation.- Call [Get comments] to obtain the comment ID'),
        }),
    },
};
...
exports.approvalV4InstanceCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            user_id: zod_1.z
                .string()
                .describe("The initiator's user_id or open_id must be passed in, with priority given to user_id if both are provided. For the way to get it, refer to [How to Obtain User ID]")
                .optional(),
            open_id: zod_1.z
                .string()
                .describe("The initiator's open_id or user_id must be passed in, with priority given to user_id if both are provided. For the way to get it, refer to [How to Obtain Open ID]")
                .optional(),
            department_id: zod_1.z
                .string()
            ...
            form: zod_1.z
                .string()
                .describe('Values of filled approval form controls must be provided as a JSON array, which needs to be compressed and escaped into a string when passed. Parameter descriptions for each control value can be found in [Approval Instance Form Control Parameters]'),
            node_approver_user_id_list: zod_1.z
                .array(zod_1.z.object({
                key: zod_1.z
                    .string()
                    .describe("The node_id or custom_node_id of the node can be obtained by calling the [View Specified Approval Definition] API and retrieving it from the node_list parameter in the interface's response")
                    .optional(),
                value: zod_1.z
                    .array(zod_1.z.string())
                    .describe('The approver list requires the user_id of users. Refer to [How to Obtain User ID] for the method to acquire user_ids')
                    .optional(),
            }))
            ...
            node_approver_open_id_list: zod_1.z
                ...
                value: zod_1.z
                    .array(zod_1.z.string())
                    .describe("The approver list requires the user open_id. Refer to [How to Obtain User's Open ID] for the method to acquire open_ids")
                    .optional(),
            }))
            ...
            node_cc_user_id_list: zod_1.z
                ...
                key: zod_1.z
                    .string()
                    .describe("The node_id for a node can be obtained by calling the [View Specified Approval Definition] API and retrieving it from the node_list parameter in the interface's response")
                    .optional(),
                value: zod_1.z
                    .array(zod_1.z.string())
                    .describe("The CC recipient list requires the user_id. Refer to [How to Obtain User's User ID] for the method to acquire user_ids")
                    .optional(),
            }))
            ...
            node_cc_open_id_list: zod_1.z
                ...
                key: zod_1.z
                    .string()
                    .describe('The node_id of a node can be obtained by calling the [View Specified Approval Definition] API and retrieving it from the node_list parameter in the response of the API')
                    .optional(),
                value: zod_1.z
                    .array(zod_1.z.string())
                    .describe("The CC recipient list requires the user's open_id. Refer to [How to Obtain User's Open ID] for the method to acquire open_ids")
                    .optional(),
            }))
            ...
            node_auto_approval_list: zod_1.z
                ...
                node_id: zod_1.z
                    .string()
                    .describe('The value of the node ID can be obtained by calling the [Get Specified Approval Definition] API. The node ID can be found in the node_list parameter of the API response')
                    .optional(),
            }))
...
exports.approvalV4InstanceGet = {
    ...
    schema: {
        params: zod_1.z
            ...
            locale: zod_1.z
                .enum(['zh-CN', 'en-US', 'ja-JP'])
                .describe('Language. The default value is the language configured with the value `true` for `is_default` in the `i18n_resources` parameter when [creating the approval definition]. Options:zh-CN(Zhcn Chinese),en-US(Enus English),ja-JP(Jajp Japanese)')
                .optional(),
            user_id: zod_1.z
                .string()
...
exports.approvalV4InstancePreview = {
    ...
    schema: {
        ...
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'user_id', 'union_id']).describe('User ID type').optional() })
            .optional(),
    },
};
exports.approvalV4InstanceQuery = {
    ...
    schema: {
        data: zod_1.z
            ...
            approval_code: zod_1.z
                .string()
                .describe('Approval Instance Code. Obtaining method:- Call the [Create Approval Instance] API and obtain the instance_code from the response parameters.- Call the [Batch Get Approval Instance ID] API to get the required approval instance code. **Note**:- user_id, approval_code, instance_code, instance_external_id, group_external_id cannot all be empty simultaneously.- Query results for instance_code and instance_external_id are a union set')
                .optional(),
            instance_code: zod_1.z
                .string()
                .describe('Approval Instance Code. Obtaining method:- Call the [Create Approval Instance] API and obtain the instance_code from the response parameters.- Call the [Batch Get Approval Instance ID] API to get the required approval instance code.**Note**:- user_id, approval_code, instance_code, instance_external_id, and group_external_id cannot all be empty simultaneously.- Query results for instance_code and instance_external_id are a union set')
                .optional(),
...
exports.approvalV4InstanceSearchCc = {
    ...
    schema: {
        data: zod_1.z
            ...
            instance_code: zod_1.z
                .string()
                .describe('Approval Instance Code. The retrieval methods are as follows:- Call the [Create Approval Instance] interface and obtain the `instance_code` from the response parameters.- Call the [Batch Get Approval Instance ID] interface to get the required approval instance code.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- Query results for `instance_code` and `instance_external_id` are obtained through union')
                .optional(),
            instance_external_id: zod_1.z
                .string()
...
exports.approvalV4InstanceSpecifiedRollback = {
    ...
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z
                .string()
                .describe('The user ID of the approver for the current approval task must match the ID type with the query parameter user_id_type. You can call [Get Single Approval Instance Details] to retrieve the user ID from the task_list parameter in the returned result, where the task status must be PENDING'),
            task_id: zod_1.z
                .string()
                .describe('The approval task ID that needs to be reverted. You can call [Get Single Approval Instance Details] to retrieve the task ID from the task_list parameter in the returned result, where the task status must be PENDING'),
            ...
            task_def_key_list: zod_1.z
                .array(zod_1.z.string())
                .describe('The node_key of the task that needs to be reverted to. You can call [Get Single Approval Instance Details] to retrieve the node_key from the timeline parameter in the returned result, where the dynamic type must be PASS'),
        }),
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() })
            .optional(),
...
exports.approvalV4TaskApprove = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            instance_code: zod_1.z
                .string()
                .describe('pproval instance Code. Acquisition methods:- After [Creating approval instance], obtain the approval instance code from the returned results.- Call [Batch acquire approval instance ID] to get the approval instance code within the specified approval definition.- Call [Query instance list] and set filtering criteria to query the specified approval instance code'),
            ...
            task_id: zod_1.z
                .string()
                .describe('Approval task ID, call [Get Single Approval Instance Details] and retrieve the required ID from the `task_list` in the returned result'),
            form: zod_1.z
                .string()
...
exports.approvalV4TaskResubmit = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            form: zod_1.z
                .string()
                .describe('Approval form control values in a JSON array, which need to be compressed and escaped into a string when passed. This parameter is used in the same way as the form parameter in [Create Approval Instance]'),
        }),
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() })
            .optional(),
...
exports.approvalV4TaskSearch = {
    ...
    schema: {
        data: zod_1.z
            ...
            instance_code: zod_1.z
                .string()
                .describe('Approval Instance Code. Methods to obtain:- After calling the [Create Approval Instance] interface, retrieve from the response parameter `instance_code`.- Call the [Batch Retrieve Approval Instance ID] interface to obtain the required approval instance Code.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- The query results for `instance_code` and `instance_external_id` are the union of their results')
                .optional(),
            instance_external_id: zod_1.z
                .string()
