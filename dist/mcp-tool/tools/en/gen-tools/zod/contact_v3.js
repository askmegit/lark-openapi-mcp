...
exports.contactV3FunctionalRoleDelete = {
    ...
    schema: {
        path: zod_1.z.object({
            role_id: zod_1.z
                .string()
                .describe('Role ID. How to obtain it:- You can obtain it from the returned result when creating a role.- Enterprise administrators can obtain the role ID on the right side of the role name in [Admin] > **Organization** > **Role** page'),
        }),
    },
};
exports.contactV3FunctionalRoleMemberBatchCreate = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            role_id: zod_1.z
                .string()
                .describe('Role ID. How to obtain it:- You can obtain it from the returned result when you [create a role].- Enterprise administrators can obtain the role ID on the right side of the role name in the [Admin] > **Organization** > **Role** page'),
        }),
    },
};
...
exports.contactV3FunctionalRoleMemberScopes = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            departments: zod_1.z
                .array(zod_1.z.string())
                .describe('Set the range of departments that the role member can manage (department ID list), and pass the value in the `["xxx", "yyy"]` array format. The ID type needs to be consistent with the value of the query parameter department_id_type.The department API provides multiple ways to obtain department IDs, such as [Get child department list], [Get parent department information], [Search department]. You can choose the appropriate API to query.**Note**: It is not supported to set the management scope of the root department (department ID is 0) for role members'),
        }),
        params: zod_1.z
            .object({
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional(),
...
exports.contactV3GroupDelete = {
    ...
    schema: {
        path: zod_1.z.object({
            group_id: zod_1.z
                .string()
                .describe('User group ID to be deleted.The user group ID can be obtained from the return value when creating the user group. You can also call the [Query User Group List] interface to obtain the user group ID'),
        }),
    },
};
exports.contactV3GroupGet = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            group_id: zod_1.z
                .string()
                .describe('User group ID.The user group ID can be obtained from the return value when creating the user group. You can also call the [Query User Group List] interface to obtain the user group ID'),
        }),
    },
};
...
exports.contactV3GroupMemberAdd = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            member_id: zod_1.z
                .string()
                .describe('The added user ID, the ID type is consistent with the value of member_id_type. For different types of ID acquisition methods, see:- [How to obtain user open_id]- [How to obtain user union_id]- [How to obtain user user_id]'),
        }),
        path: zod_1.z.object({
            group_id: zod_1.z
                .string()
    ...
    schema: {
        data: zod_1.z.object({
            ...
            parent_job_family_id: zod_1.z
                .string()
                .describe('Parent job family ID. If you need to add a sub job family to a job family, you need to pass in this parameter value. You can call the [Get tenant job family list] interface to get the job family ID.**Default value**: empty, indicating that the job family has no parent job family')
                .optional(),
    ...
    schema: {
        path: zod_1.z.object({
            job_family_id: zod_1.z
                .string()
                .describe('Job family ID. How to obtain:- You can obtain it from the returned result when [Create job family].- Call the [Get tenant Job family list] interface to obtain the Job family ID'),
        }),
    },
};
...
exports.contactV3JobFamilyUpdate = {
    ...
    schema: {
        data: zod_1.z
            ...
            parent_job_family_id: zod_1.z
                .string()
                .describe('Parent job family ID. You can call the [Get tenant job family list] interface to get the job family ID.**Default value**: empty, indicating no update')
                .optional(),
            status: zod_1.z
                .boolean()
    ...
    schema: {
        path: zod_1.z.object({
            job_level_id: zod_1.z
                .string()
                .describe('Job level ID. How to obtain it:- When creating a job level, you can obtain the job level ID from the returned result.- Call the [Get tenant job level list] interface to find the ID information of the specified job level'),
        }),
    },
};
...
exports.contactV3ScopeList = {
    ...
    schema: {
        params: zod_1.z
            ...
            department_id_type: zod_1.z
                .enum(['department_id', 'open_department_id'])
                .describe('Type of the returned department ID. For an introduction to types, see [General Parameters]. Options:department_id(Supports user-defined configured department IDs. The deleted department_id can be reused when customizing the configuration, so department_id is unique within the scope of the department that has not been deleted.),open_department_id(Department ID automatically generated by the system. The ID prefix is fixed to `od-` and is globally unique within the tenant.)')
                .optional(),
            page_token: zod_1.z
                .string()
                .describe('Page identifier. It is not filled in the first request, indicating traversal from the beginning when there will be more groups, the new page_token will be returned at the same time, and the next traversal can use the page_token to get more groups')
...
exports.contactV3UnitBindDepartment = {
    ...
    schema: {
        data: zod_1.z.object({
            unit_id: zod_1.z
                .string()
                .describe('Unit ID.When you create a unit, you can get the unit ID in the returned result. You can also call the [Get Unit List] interface to get the unit ID'),
            department_id: zod_1.z
                .string()
                .describe('The department ID associated with the unit. The ID type is consistent with the value of department_id_type.The department API provides multiple ways to obtain department IDs, such as [Get child department list], [Get parent department information], and [Search for department]. You can choose the appropriate API to query'),
            department_id_type: zod_1.z
                .enum(['department_id', 'open_department_id'])
                .describe('The department ID type in this call. For a detailed description of department IDs, see [Department ID Description]. Options:department_id(Supports user-defined department ID. When customizing the configuration, the deleted department_id can be reused, so the department_id is unique within the scope of the non-deleted department.),open_department_id(The department ID is automatically generated by the system. The ID prefix is ​​fixed to `od-` and is globally unique within the tenant.)')
                .optional(),
...
exports.contactV3UserCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            avatar_key: zod_1.z
                .string()
                .describe('The file key of the avatar. You can upload and obtain the avatar file Key through the [Upload Image] interface. When uploading, you need to select for setting avatar as the picture type**Default value**: empty')
                .optional(),
            department_ids: zod_1.z
                .array(zod_1.z.string())
            ...
            leader_user_id: zod_1.z
                .string()
                .describe("The user ID of the user's direct supervisor, the ID type is consistent with the query parameter `user_id_type`. For how to obtain user ID, please refer to [How to obtain different user IDs].**Default value**: empty")
                .optional(),
            city: zod_1.z
                .string()
            ...
            country: zod_1.z
                .string()
                .describe('Country or region code. Refer to the [list of country/region codes] for details.**Default value**: empty')
                .optional(),
            ...
            employee_type: zod_1.z
                .number()
                .describe("Employee type. **Optional values are**:- 1: Regular- 2: Intern- 3: Outsourcing- 4: Contractor- 5: Consultant This parameter supports reading int values of custom employee types. You can obtain the number value (enum_value) of the tenant's custom employee type through the [Get Personnel Type] interface"),
            orders: zod_1.z
                .array(zod_1.z.object({
                department_id: zod_1.z
                    .string()
            ...
            custom_attrs: zod_1.z
                ...
                value: zod_1.z
                    ...
                    option_id: zod_1.z
                        .string()
                        .describe('This parameter is used to define the option ID when the custom field type is `ENUMERATION` or `PICTURE_ENUM`.You can call the [Get enterprise custom user fields] interface to obtain the corresponding option ID of the custom field')
                        .optional(),
                    generic_user: zod_1.z
                        .object({
                        id: zod_1.z
                            .string()
                            .describe('The user ID of the referrer.- The ID type is consistent with the query parameter `user_id_type`.- For how to obtain the user ID, please refer to [How to obtain different user IDs]'),
                        type: zod_1.z
                            .number()
            ...
            job_level_id: zod_1.z
                .string()
                .describe('Job level. You can call the [Get tenant job level list] interface to query the corresponding job level.**Default**: empty')
                .optional(),
            job_family_id: zod_1.z
                .string()
                .describe('Job family. You can call the [Get tenant job family list] interface to query the corresponding job family.**Default**: empty')
                .optional(),
            subscription_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('List of seat IDs assigned to the user.**Precautions**:- This field requires the **Assign User Seatst** permission.- If you purchased a seat, you must assign a seat ID to the user when creating the user, otherwise the user will not be able to log in to Feishu Enterprise. For more information, please see [Administrator allocation of seats].- You can obtain the available seat ID of the current tenant through the [Get Enterprise Seat Information] interface.**Default**: empty')
                .optional(),
            dotted_line_leader_user_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('List of user IDs for dotted superiors.- The ID type is consistent with the query parameter `user_id_type`.- For how to obtain the user ID, please refer to [How to obtain different user IDs].**Default**: empty')
                .optional(),
        }),
        params: zod_1.z
            .object({
    ...
    schema: {
        data: zod_1.z
            ...
            department_chat_acceptor_user_id: zod_1.z
                .string()
                .describe('The user ID of the department group recipient. When the deleted user is the group owner of a department group, the group owner will be transferred to the designated recipient.**Notice**:- The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].- When this parameter is not specified, if the deleted user is the group owner of a department group, the group owner will be transferred to the first person in the group to join the group by default')
                .optional(),
            external_chat_acceptor_user_id: zod_1.z
                .string()
                .describe('The user ID of the external group recipient. When the deleted user is the owner of an external group, the group owner will be transferred to the designated recipient.**Notice**:- The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].- When this parameter is not specified, if the deleted user is the owner of an external group, the group owner will be transferred to the first person in the group who is in the same organization as the deleted user by default. If only the deleted user is in the group within the organization, the external group will be disbanded')
                .optional(),
            docs_acceptor_user_id: zod_1.z
                .string()
                .describe('The user ID of the recipient of the document. When a user is deleted, the documents they own are transferred to the recipient.**Notice**:- The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].- If no recipient is specified, it will be transferred to the immediate superior of the deleted user by default. If the deleted user has no immediate superior, the document resources will remain under the user name.- After the document is transferred, only the document owner is changed, and other permissions are not affected')
                .optional(),
            ...
            application_acceptor_user_id: zod_1.z
                .string()
                .describe('The user ID of the recipient of the application. When a user is deleted, the applications they created are transferred to the recipient.**Notice**:- The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].- If no recipient is specified, it will be transferred to the immediate superior of the deleted user by default. If the deleted user has no immediate superior, the application will remain under the user name, but the user will not be able to log in to the developer backend for application management. Enterprise administrators can manually transfer applications to others in the management background')
                .optional(),
            minutes_acceptor_user_id: zod_1.z
                .string()
                .describe("Minute recipient's user ID. When a user is deleted, the Minute resources they own are transferred to the recipient.**Notice**:- The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].- If no recipient is specified, it will be transferred to the immediate superior of the deleted user by default. If the deleted user has no immediate superior, Minute will remain under the user name.- After the Minute is transferred, only the Minute owner is changed, and the shared Minute link is not affected")
                .optional(),
            survey_acceptor_user_id: zod_1.z
                .string()
                .describe('The user ID of the survey recipient. When a user is deleted, the Survey resources they own are transferred to the recipient.**Notice**:- The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].- If no recipient is specified, it will be transferred to the immediate superior of the deleted user by default. If the deleted user has no immediate superior, the Survey resource will be deleted directly')
                .optional(),
            email_acceptor: zod_1.z
                ...
                acceptor_user_id: zod_1.z
                    .string()
                    .describe('The user ID of the recipient of the mail resource. The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs].**Note**: This parameter value needs to be set only when the value of `processing_type` is `1`')
                    .optional(),
            })
            ...
            anycross_acceptor_user_id: zod_1.z
                .string()
                .describe("The user ID of the recipient of the user integration platform resources.** Attention: **- The ID type needs to be consistent with the user_id_type type in the query parameters. For how to obtain user ID, please refer to [How to obtain different user IDs].- If the recipient is not specified, it will be transferred to the deleted user's direct manager by default. If the deleted user does not have a direct manager, the application will remain under the user's name, but the user cannot log in to the Feishu integration platform. Enterprise administrators can manually transfer applications to others in the administrative background")
                .optional(),
        })
            .optional(),
        params: zod_1.z
        ...
        path: zod_1.z.object({
            user_id: zod_1.z
                .string()
                .describe('User ID, which must match the user_id_type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs]'),
        }),
    },
};
exports.contactV3UserFindByDepartment = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            department_id_type: zod_1.z
                .enum(['department_id', 'open_department_id'])
                .describe('The type of department ID used in this call. For a detailed introduction to department ID, please refer to [Department ID Description]. Options:department_id(Supports user-defined configured department IDs. The deleted department_id can be reused when customizing the configuration, so department_id is unique within the scope of the department that has not been deleted.),open_department_id(Department ID automatically generated by the system. The ID prefix is fixed to `od-` and is globally unique within the tenant.)')
                .optional(),
            department_id: zod_1.z
                .string()
                .describe('Department ID, ID type is consistent with the value of department_id_type.**Note**:- The root department has a department ID of 0.- You can call the [Search Department] interface to obtain the corresponding department ID through the department name keyword'),
            page_size: zod_1.z
                .number()
...
exports.contactV3UserList = {
    ...
    path: '/open-apis/contact/v3/users',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-Deprecated Version (Not Recommended)-Contact-User-Get User List-Obtain the list of users directly under the department based on the department ID.[FAQs]',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z
            .object({
...
exports.contactV3UserPatch = {
    ...
    schema: {
        data: zod_1.z
            ...
            avatar_key: zod_1.z
                .string()
                .describe('The file key of the avatar. You can upload and obtain the avatar file Key through the [Upload Image] interface. When uploading, you need to select for setting avatar as the picture type')
                .optional(),
            department_ids: zod_1.z
                .array(zod_1.z.string())
            ...
            leader_user_id: zod_1.z
                .string()
                .describe("The user ID of the user's direct supervisor, the ID type is consistent with the value of the query parameter `user_id_type`. For how to obtain user ID, please refer to [How to obtain different user IDs]")
                .optional(),
            city: zod_1.z
                .string()
            ...
            employee_type: zod_1.z
                .number()
                .describe('Employee type. Optional values are:- `1`: Regular- `2`: Intern- `3`: Outsourcing- `4`: Contractor- `5`: Consultant The int value of the custom employee type can also be read, and the name of the custom employee type for the tenant can be obtained through the API: [Obtain workforce type]')
                .optional(),
            orders: zod_1.z
                .array(zod_1.z.object({
                department_id: zod_1.z
            ...
            custom_attrs: zod_1.z
                ...
                value: zod_1.z
                    ...
                    generic_user: zod_1.z
                        ...
                        id: zod_1.z
                            .string()
                            .describe('The user ID of the referrer.- The personnel ID involved in the custom field only supports the identifier in the user_id format, and this restriction is not affected by the value of user_id_type in the query parameters. - For how to obtain the user ID, please refer to [How to obtain different user IDs]'),
                        type: zod_1.z
                            .number()
                            .describe('user type.**Optional values are**:1: User**Note**: Currently only the value 1 is supported, which means user'),
                    })
                ...
                .describe('Custom fields. Before setting parameters, it is recommended that you first understand [Custom field resource introduction].**Note**: This field will only take effect when the enterprise administrator configures a custom field in the management background and turns on the **Allow Open Platform API Call** function')
                .optional(),
            enterprise_email: zod_1.z
                .string()
            ...
            job_level_id: zod_1.z
                .string()
                .describe('Job level ID. You can call the [Get tenant job level list] interface to query the corresponding job level ID')
                .optional(),
            job_family_id: zod_1.z
                .string()
                .describe('Job family ID. You can call the [Get tenant job family list] interface to query the corresponding job family ID')
                .optional(),
            subscription_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('List of seat IDs assigned to the user.**Precautions**:- This field requires the **Assign seat for user** permission.- You can obtain the available seat ID of the current tenant through the [Get enterprise seat information] interface')
                .optional(),
            dotted_line_leader_user_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('List of user IDs for dotted superiors.- The ID type is consistent with the value of the query parameter `user_id_type`.- For how to obtain the user ID, please refer to [How to obtain different user IDs]')
                .optional(),
        })
            .optional(),
        params: zod_1.z
...
exports.contactV3UserResurrect = {
    ...
    schema: {
        data: zod_1.z
            ...
            subscription_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('If the user was assigned a [seat] in normal status, this parameter can be used to specify the seat ID assigned post-recovery. For information on how to obtain the assigned seat ID, refer to the [Get Enterprise Seat Information API].**Notice**:- This field requires the **Assign seat for user** permission')
                .optional(),
        })
            .optional(),
        params: zod_1.z
        ...
        path: zod_1.z.object({
            user_id: zod_1.z
                .string()
                .describe('User ID. The ID type needs to be consistent with the user_id_type type in the query parameter. For how to obtain user ID, please refer to [How to obtain different user IDs]'),
        }),
    },
};
exports.contactV3UserUpdate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            avatar_key: zod_1.z
                .string()
                .describe('File key of the profile photo. You can upload an image by calling the "Upload Image" API in "Messenger > Message > Images" to obtain the file key.The document refer to [upload images]')
                .optional(),
            department_ids: zod_1.z
                .array(zod_1.z.string())
                .describe("List of IDs of the user's departments. A user can belong to multiple departments.The ID value matches the department_id_type in the query parameter.For information about different IDs, see [Department IDs]"),
            leader_user_id: zod_1.z
                .string()
                .describe("User ID of the user's direct manager. The ID value matches the user_id_type in the query parameter.For information about different IDs, see [User-related IDs]")
                .optional(),
            ...
            orders: zod_1.z
                ...
                department_id: zod_1.z
                    .string()
                    .describe('Department ID for the sorting information. The ID value matches the department_id_type in the query parameter.For information about different IDs, see [Department IDs]')
                    .optional(),
                user_order: zod_1.z
                    .number()
            ...
            custom_attrs: zod_1.z
                ...
                type: zod_1.z
                    .string()
                    .describe('Custom field type - `TEXT`: Text- `HREF`: Webpage- `ENUMERATION`: Enumeration- `PICTURE_ENUM`: Image- `GENERIC_USER`: User[FAQs about custom fields]')
                    .optional(),
                id: zod_1.z.string().describe('Custom field ID').optional(),
                value: zod_1.z
                    .object({
                ...
                .describe('Custom field. Make sure your company administrator has enabled "Allow to invoke by Open Platform\'s Contacts API" in Admin > Organization > Member Profile > Custom Fields > Global Settings; otherwise, this field will not take effect or be returned.more detail see [FQAs]')
                .optional(),
            enterprise_email: zod_1.z
                .string()
                .describe('Business email. Make sure that Feishu Business Mail has been enabled in Admin.When create user, the detail of enterprise email see [FQAs]')
                .optional(),
