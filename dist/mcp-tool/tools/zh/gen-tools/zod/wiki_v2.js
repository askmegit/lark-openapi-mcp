    ...
    schema: {
        params: zod_1.z
            ...
            lang: zod_1.z
                ...
                .describe('当查询**我的文档库**时，指定返回的文档库名称展示语言。 Options:zh(LangZH 简体中文),id(LangID 印尼语),de(LangDE 德语),en(LangEN 英语),es(LangES 西班牙语),fr(LangFR 法语),it(LangIT 意大利语),pt(LangPT 葡萄牙语),vi(LangVI 越南语),ru(LangRU 俄语),hi(LangHI 印地语),th(LangTH 泰语),ko(LangKO 韩语),ja(LangJA 日语),zh-HK(LangZHHK 繁体中文（中国香港）),zh-TW(LangZHTW 繁体中文（中国台湾）)')
                .optional(),
        })
            .optional(),
        path: zod_1.z
            ...
            space_id: zod_1.z
                .string()
                .describe('知识空间 ID。可通过以下两种方式获取。了解更多，参考[知识库概述]。- 调用 [获取知识空间列表]获取- 如果你是知识库管理员，可以进入知识库设置页面，复制地址栏的数字部分：https://sample.feishu.cn/wiki/settings/==6870403571079249922==')
                .optional(),
        })
            .optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.wikiV2SpaceGetNode = {
    ...
    schema: {
        params: zod_1.z.object({
            token: zod_1.z
                .string()
                .describe('知识库节点或对应云文档的实际 token。- 知识库节点 token：如果 URL 链接中 token 前为 wiki，该 token 为知识库的节点 token。- 云文档实际 token：如果 URL 链接中 token 前为 docx、base、sheets 等非 wiki 类型，则说明该 token 是当前云文档的实际 token。了解更多，请参考[文档常见问题-如何获取云文档资源相关 token（id）]。**注意**：使用云文档 token 查询时，需要对 obj_type 参数传入文档对应的类型'),
            obj_type: zod_1.z
...
exports.wikiV2SpaceMemberCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                .string()
                .describe('要添加的成员或管理员的身份类型。可选值：- openchat：群组 ID。参考[群 ID 说明]获取- userid：用户 ID。详情参考[如何获取 User ID]- email：用户邮箱- opendepartmentid：部门 ID。参考[部门资源介绍]获取- openid：用户的 Open ID。详情参考[如何获取 Open ID]- unionid：用户的 Union ID。详情参考[如何获取 Union ID]'),
            member_id: zod_1.z
                .string()
        ...
        params: zod_1.z.object({ need_notification: zod_1.z.boolean().describe('添加权限后是否通知对方').optional() }).optional(),
        path: zod_1.z
            .object({
            space_id: zod_1.z
                .string()
...
exports.wikiV2SpaceMemberDelete = {
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                .string()
                .describe('“openchat” - 群id “userid” - 用户id“email” - 邮箱“opendepartmentid” - 部门id“openid” - 应用openid“unionid” - [unionid]'),
    ...
    schema: {
        data: zod_1.z.object({
            obj_type: zod_1.z
                ...
                .describe('文档类型，对于快捷方式，该字段是对应的实体的obj_type。 Options:doc(ObjTypeDoc 已废弃，创建文档请使用`docx`。详情参考[旧版文档（Docs 1.0）创建能力下线说明]。),sheet(ObjTypeSheet 表格),mindnote(ObjTypeMindNote 思维导图),bitable(ObjTypeBitable 多维表格),file(ObjTypeFile 文件),docx(ObjTypeDocx 新版文档),slides(ObjTypeSlides 幻灯片)'),
        ...
        path: zod_1.z.object({ space_id: zod_1.z.string().describe('知识空间id[获取方式]').optional() }).optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2SpaceNodeList = {
    ...
    schema: {
        params: zod_1.z
            ...
            .optional(),
        path: zod_1.z.object({ space_id: zod_1.z.string().describe('知识空间id').optional() }).optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2SpaceNodeMove = {
...
exports.wikiV2TaskGet = {
    ...
    schema: {
        params: zod_1.z.object({
            task_type: zod_1.z.literal('move').describe('任务类型 Options:move([移动云空间文档至知识空间]任务)'),
        }),
        path: zod_1.z.object({ task_id: zod_1.z.string().describe('任务id').optional() }).optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2Tools = [
