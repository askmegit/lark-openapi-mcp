...
exports.performanceV2ActivityQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-绩效-后台配置-周期与项目-项目-获取项目列表-批量获取项目的配置信息，如项目名称、项目模式等信息',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z
            .object({
...
exports.performanceV2AdditionalInformationImport = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-绩效-后台配置-周期与项目-补充信息-批量导入补充信息-批量导入被评估人的补充信息作为绩效评估的参考，如补充信息的事项、时间以及具体描述等。该接口支持创建和更新补充信息',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            semester_id: zod_1.z.string().describe('评估周期 ID，可通过[获取周期列表]接口获取'),
            additional_informations: zod_1.z
                ...
                reviewee_user_id: zod_1.z
                    .string()
                    .describe('被评估人 ID 列表，与入参 `user_id_type` 类型一致，可通过[获取被评估人信息]接口获取**注意**：若 `reviewee_user_id`、`item `、`time `、`detailed_description` 的参数组合在系统中已存在内容一致的补充消息时，将更新对应的补充消息数据'),
                item: zod_1.z
                    .string()
...
exports.performanceV2AdditionalInformationQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-绩效-后台配置-周期与项目-补充信息-批量查询补充信息-批量查询被评估人的补充信息，如补充信息的事项、时间以及具体描述等',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            semester_id: zod_1.z
                .string()
                .describe('评估周期 ID，可通过[获取周期列表]接口获取**注意**：若请求参数 `item_ids`、`external_ids`、`reviewee_user_ids` 均为空，返回 `semester_id` 参数指定周期的所有补充信息'),
            item_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('补充信息 ID 列表，可通过[批量导入补充信息]接口获取**说明**：若提供多个筛选参数，按照 `item_ids` > `external_ids` > `reviewee_user_ids` 的优先级顺序，以第一个有值的筛选参数进行筛选')
                .optional(),
            external_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('外部系统补充信息 ID 列表，该 ID 在通过[批量导入补充信息]接口导入时写入**说明**：若提供多个筛选参数，按照 `item_ids` > `external_ids` > `reviewee_user_ids` 的优先级顺序，以第一个有值的筛选参数进行筛选')
                .optional(),
            reviewee_user_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('被评估人 ID 列表，与入参 `user_id_type` 类型一致，可通过[获取被评估人信息]接口获取**说明**：若提供多个筛选参数，按照 `item_ids` > `external_ids` > `reviewee_user_ids` 的优先级顺序，以第一个有值的筛选参数进行筛选')
                .optional(),
        }),
        params: zod_1.z
            .object({
...
exports.performanceV2MetricDetailImport = {
    ...
    schema: {
        data: zod_1.z.object({
            semester_id: zod_1.z.string().describe('周期 ID，可通过[获取周期]接口获取'),
            ...
            imported_metrics: zod_1.z
                ...
                metric_id: zod_1.z.string().describe('指标 ID，可通过[获取指标列表]接口获取'),
                fields: zod_1.z
                    .array(zod_1.z.object({
                    field_id: zod_1.z.string().describe('指标字段 ID，可通过[获取指标字段列表]接口获取'),
...
exports.performanceV2MetricDetailQuery = {
    ...
    schema: {
        data: zod_1.z.object({
            semester_id: zod_1.z.string().describe('周期 ID，1 次只允许查询 1 个周期，semester_id 可通过【[获取周期]】接口获得'),
...
exports.performanceV2MetricLibQuery = {
    ...
    schema: {
        data: zod_1.z
            ...
            type_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('指标类型 ID 列表，可通过[获取指标模板列表]接口返回结果中的 `data.items.metrics.type_id` 获取，填写时根据指定的指标类型进行筛选，type_ids的长度范围（0～99）')
                .optional(),
...
exports.performanceV2ReviewDataQuery = {
    ...
    schema: {
        data: zod_1.z.object({
            semester_ids: zod_1.z.array(zod_1.z.string()).describe('评估周期 ID 列表，semester_id 可通过[【获取周期】]获得'),
            ...
            stage_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('环节 ID，如果同时传了环节 ID 和环节类型，优先返回环节 ID 对应的绩效数据。stage_types 和 stage_ids 至少要传一个，不传默认不返回任何环节评估数据。可在事件[绩效结果开通]、[绩效详情变更]获得，用于接收事件后按环节查询评估数据场景')
                .optional(),
...
exports.performanceV2ReviewTemplateQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-绩效-后台配置-评估模板和评估题-获取绩效模板配置-获取绩效模板信息，包括模版名称、执行角色、填写项类型等',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z
            .object({
...
exports.performanceV2RevieweeQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-绩效-后台配置-周期与项目-员工信息-获取被评估人信息-获取绩效周期中被圈定到项目中的被评估人信息，包括未启动的项目',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            semester_id: zod_1.z.string().describe('周期 ID，可通过[获取周期列表]接口获取'),
            user_ids: zod_1.z
                .array(zod_1.z.string())
