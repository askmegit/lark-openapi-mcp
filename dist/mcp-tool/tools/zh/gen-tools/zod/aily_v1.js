...
exports.ailyV1AilySessionAilyMessageCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书 Aily-消息-发送 Aily 消息-该 API 用于向某个飞书 Aily 应用发送一条消息（Message）；每个消息从属于一个活跃的会话（Session）',
    ...
    schema: {
        data: zod_1.z.object({
            ...
            content_type: zod_1.z
                ...
                .describe('消息的类型，包括 `MDX` | `TEXT` 等- `MDX` 能够表达富文本信息结构，可参考 [Aily 消息节点 markdown 语法]- `TEXT` 作为纯文本进行处理 Options:MDX(ContentTypeMDX MDX),TEXT(ContentTypeText TEXT),CLIP(ContentTypeClip GUI 卡片),SmartCard(ContentTypeSmartCard SmartCard),JSON(ContentTypeJSON JSON)'),
            ...
            mentions: zod_1.z
                ...
                identity_provider: zod_1.z
                    .enum(['AILY', 'FEISHU'])
                    .describe('身份提供者 Options:AILY(IdentityProviderAily Aily 账号体系),FEISHU(IdentityProviderFeishu 飞书账号体系)')
                    .optional(),
        ...
        path: zod_1.z.object({ aily_session_id: zod_1.z.string().describe('会话 ID；参考 [创建会话] 接口') }),
...
exports.ailyV1AilySessionAilyMessageGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书 Aily-消息-获取 Aily 消息-该 API 用于获取某个飞书 Aily 应用的消息（Message）的详细信息；包括消息的内容、发送人等',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({
            aily_session_id: zod_1.z.string().describe('会话 ID；参考 [创建会话] 接口'),
...
exports.ailyV1AilySessionAilyMessageList = {
    ...
    path: '/open-apis/aily/v1/sessions/:aily_session_id/messages',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书 Aily-消息-列出 Aily 消息-该 API 用于列出某个飞书 Aily 应用的某个会话（Session）下消息（Message）的详细信息；包括消息的内容、发送人等',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z
            .object({
...
exports.ailyV1AilySessionCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书 Aily-会话-创建会话-该 API 用于创建与某个飞书 Aily 应用的一次会话（Session）；当创建会话成功后，可以发送消息、创建运行',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z
            .object({
            channel_context: zod_1.z
                .string()
...
exports.ailyV1AilySessionGet = {
    ...
    path: '/open-apis/aily/v1/sessions/:aily_session_id',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书 Aily-会话-获取会话-该 API 用于获取与某个飞书 Aily 应用的一次会话（Session）的详细信息，包括会话的状态、渠道上下文、创建时间等',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({ aily_session_id: zod_1.z.string().describe('会话 ID') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.ailyV1AilySessionRunCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书 Aily-运行-创建运行-该 API 用于在某个飞书 Aily 应用会话（Session）上创建一次运行（Run）',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
...
exports.ailyV1AilySessionRunGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书 Aily-运行-获取运行-该 API 用于获取某个飞书 Aily 应用的运行（Run）的详细信息；包括运行的状态、结束时间等',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({
            aily_session_id: zod_1.z.string().describe('会话 ID；参考 [创建会话] 接口'),
...
exports.ailyV1AilySessionRunList = {
    ...
    path: '/open-apis/aily/v1/sessions/:aily_session_id/runs',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书 Aily-运行-列出运行-该 API 用于列出某个飞书 Aily 应用的运行（Run）的详细信息；包括状态、结束时间等',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z
            .object({
...
exports.ailyV1AppDataAssetCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            connect_type: zod_1.z.enum(['import', 'direct']).describe('连接类型 Options:import(导入模式),direct(直连模式)'),
            source_type: zod_1.z
                .enum(['file', 'lark_wiki_space', 'lark_doc', 'lark_helpdesk'])
                .describe('数据源类型 Options:file(文件，只支持导入模式),lark_wiki_space(飞书知识空间，只支持直连模式),lark_doc(飞书云文档，导入模式只支持docx),lark_helpdesk(飞书服务台，只支持直连模式)'),
            import_knowledge_setting: zod_1.z
                ...
                chunk_setting: zod_1.z
                    ...
                    separate_type: zod_1.z
                        ...
                        .optional(),
                    size: zod_1.z.number().describe('分段最大长度（字符），按标识符切片时必须填写').optional(),
                    overlap: zod_1.z.number().describe('分段重叠字符数，按标识符切片时必须填写，不能超过size的数值').optional(),
                })
                    .describe('知识切片配置')
                    .optional(),
                file: zod_1.z
                    .object({
                    title: zod_1.z.string().describe('文件标题').optional(),
                    token: zod_1.z.string().describe('上传文件获取到的token。和content二选一，优先使用token').optional(),
                    content: zod_1.z
                        .string()
                    ...
                    mime_type: zod_1.z
                        .string()
                        .describe('文件内容对应的 MIME 类型，必须填写可选值：- text/plain (.txt)- application/pdf (.pdf)- application/vnd.openxmlformats-officedocument.presentationml.presentation (.pptx)- application/vnd.openxmlformats-officedocument.wordprocessingml.document (.docx)')
                        .optional(),
                    url: zod_1.z.string().describe('文件源的URL').optional(),
                })
                    .describe('知识导入-文件')
                    .optional(),
                lark_doc: zod_1.z
                    ...
                    type: zod_1.z
                        ...
                        .describe('云文档类型 Options:doc(飞书文档),file(飞书文件),wiki(飞书知识库),docx(飞书新版文档),folder(飞书文件夹)'),
                    token: zod_1.z.string().describe('云文档token，可以通过[搜索云文档]API获取'),
                    with_sub_docs: zod_1.z.boolean().describe('是否包含子文档，只有wiki类型的云文档支持').optional(),
                })
                    .describe('知识导入-飞书云文档')
                    .optional(),
                lark_wiki_space: zod_1.z
                    .object({
                    space_id: zod_1.z.string().describe('飞书知识空间ID，可以通过[搜索 Wiki]API获取'),
                    sub_docs: zod_1.z
                        .array(zod_1.z.object({
                        type: zod_1.z.literal('wiki').describe('云文档类型，只支持wiki中的云文档 Options:wiki(飞书知识库)'),
                        token: zod_1.z.string().describe('云文档token，可以通过[搜索云文档]API获取'),
                        url: zod_1.z.string().describe('云文档链接').optional(),
                    }))
                        .describe('指定知识空间子节点时使用')
                        .optional(),
                    url: zod_1.z.string().describe('知识空间URL').optional(),
                })
                    .describe('知识导入-飞书知识空间')
                    .optional(),
                lark_helpdesk: zod_1.z
                    .object({ helpdesk_id: zod_1.z.string().describe('飞书服务台ID，可以通过[服务台-接入指南] 获取') })
                    .describe('知识导入-飞书服务台')
                    .optional(),
            })
                .describe('知识导入配置')
                .optional(),
            description: zod_1.z.object({}).catchall(zod_1.z.any()).describe('数据知识描述信息').optional(),
        }),
        params: zod_1.z
            .object({
            tenant_type: zod_1.z
            ...
            .optional(),
        path: zod_1.z.object({
            app_id: zod_1.z.string().describe('Aily 平台的应用的APPID，可以直接从 Aily 应用的URL中获取。获取示例：/ai/{APPID}'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.ailyV1AppDataAssetDelete = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            app_id: zod_1.z.string().describe('Aily 平台的应用的APPID，可以直接从 Aily 应用的URL中获取。获取示例：/ai/{APPID}'),
            data_asset_id: zod_1.z
                .string()
                .describe('数据知识ID，可通过在 Aily 平台查看知识详情页的url中获取，获取示例 https://***/ai/app_namespace/data/data-asset/data_asset_id'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.ailyV1AppDataAssetGet = {
    ...
    schema: {
        params: zod_1.z
            .object({
            with_data_asset_item: zod_1.z.boolean().describe('结果是否包含数据与知识项').optional(),
            with_connect_status: zod_1.z.boolean().describe('结果是否包含数据知识连接状态').optional(),
            tenant_type: zod_1.z.string().describe('应用环境，默认为线上环境，dev代表开发环境').optional(),
        })
            .optional(),
        path: zod_1.z.object({
            app_id: zod_1.z.string().describe('Aily 平台的应用的APPID，可以直接从 Aily 应用的URL中获取。获取示例：/ai/{APPID}'),
...
exports.ailyV1AppSkillList = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书 Aily-技能-查询技能列表-该 API 用于查询某个 Aily 应用的技能列表> 包括内置的数据分析与问答技能、以及未在对话开启的技能',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z
            .object({
...
exports.ailyV1AppSkillStart = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书 Aily-技能-调用技能-该 API 用于调用某个 Aily 应用的特定技能，支持指定技能入参；并同步返回技能执行的结果',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z
            .object({
            global_variable: zod_1.z
                ...
                channel: zod_1.z
                    .object({
                    variables: zod_1.z.string().describe('自定义传入的变量；可在 Workflow 技能全局变量中消费').optional(),
                })
                    .describe('渠道信息')
                    .optional(),
            })
