...
exports.trustPartyV1CollaborationTenantCollaborationDepartmentGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-关联组织-关联组织-获取关联组织部门详情-获取对方的关联组织部门详情，需要对部门有可见性权限才可以获取',
    ...
    schema: {
        params: zod_1.z
            ...
            .optional(),
        path: zod_1.z.object({
            target_tenant_key: zod_1.z.string().describe('对方关联组织的tenant key，可通过[管理员获取所有关联组织列表]获取'),
...
exports.trustPartyV1CollaborationTenantCollaborationUserGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-关联组织-关联组织-获取关联组织成员详情-获取关联组织成员详情，需要对关联组织成员有权限才可以获取',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z
            .object({
            target_user_id_type: zod_1.z
                ...
                .describe('用户ID类型，可以在[获取关联组织的部门和成员信息]中获取对应的用户ID Options:user_id(以user_id来识别用户),union_id(以union_id来识别用户),open_id(以open_id来识别用户)')
                .optional(),
        })
            .optional(),
        path: zod_1.z.object({
            target_tenant_key: zod_1.z.string().describe('对方关联组织的tenant key，可通过[管理员获取所有关联组织列表]获取'),
            target_user_id: zod_1.z.string().describe('请求的关联组织用户ID，需要与target_user_id_type中填写的类型保持一致'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.trustPartyV1CollaborationTenantGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-关联组织-关联组织-获取关联组织详情-基于组织tenant key获取关联组织详情，需要对对方组织有可见权限才可以获取',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({
            target_tenant_key: zod_1.z.string().describe('对方关联组织的tenant key，可通过[管理员获取所有关联组织列表]获取'),
    ...
    schema: {
        params: zod_1.z
            ...
            page_token: zod_1.z
                ...
                .optional(),
            page_size: zod_1.z.number().describe('单次请求的关联组织数量').optional(),
        })
            .optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.trustPartyV1CollaborationTenantVisibleOrganization = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-关联组织-关联组织-获取关联组织的部门和成员信息-该接口会返回用户在外部部门下可见的下级部门、用户、用户组',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z
            .object({
            ...
            target_department_id: zod_1.z
                .string()
                .describe('请求关联组织的部门ID，0代表根部门，与target_group_id二选一；可以从[获取关联组织的部门和成员信息]中获得')
                .optional(),
            page_token: zod_1.z
                .string()
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
                .optional(),
            page_size: zod_1.z.number().describe('请求的可见实体数量').optional(),
            group_id_type: zod_1.z
                .enum(['group_id', 'open_group_id'])
                .describe('此次调用中使用的用户组ID的类型 Options:group_id(group_id),open_group_id(以open_group_id来标识用户组)')
                .optional(),
            ...
            .optional(),
        path: zod_1.z.object({
            target_tenant_key: zod_1.z.string().describe('对方关联组织的 tenant key。可通过[管理员获取所有关联组织列表]获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
