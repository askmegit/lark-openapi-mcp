...
exports.workplaceV1CustomWorkplaceAccessDataSearch = {
    ...
    schema: {
        params: zod_1.z.object({
            from_date: zod_1.z.string().describe('数据检索开始时间，精确到日。格式yyyy-MM-dd'),
            to_date: zod_1.z.string().describe('数据检索结束时间，精确到日。格式yyyy-MM-dd'),
            page_size: zod_1.z.number().describe('分页大小，最小为 1，最大为 200，默认为 20'),
            page_token: zod_1.z
                .string()
            ...
            custom_workplace_id: zod_1.z
                .string()
                .describe('定制工作台id，非必填。不填时，返回所有定制工作台数据。如何获取定制工作台ID：可前往 飞书管理后台 > 工作台 > 定制工作台，点击指定工作台的 设置 进入设置页面；鼠标连续点击三次顶部的 设置 字样即可出现 ID，复制 ID 即可')
                .optional(),
        }),
    },
};
exports.workplaceV1WorkplaceAccessDataSearch = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-工作台-工作台访问数据-获取工作台访问数据-获取工作台访问数据（包含默认工作台与定制工作台）',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            from_date: zod_1.z.string().describe('数据检索开始时间，精确到日。格式yyyy-MM-dd'),
...
exports.workplaceV1WorkplaceBlockAccessDataSearch = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            block_id: zod_1.z
                .string()
                .describe('小组件id（BlockID）。可前往 飞书管理后台 > 工作台 > 定制工作台，选择指定的工作台并进入工作台编辑器，点击某个小组件，即可查看页面右侧面板中该小组件名称下方的“BlockID”')
                .optional(),
        }),
    },
};
