...
exports.cardkitV1CardBatchUpdate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书卡片-卡片-局部更新卡片实体-更新卡片实体局部内容，包括配置和组件。支持同时对多个组件进行增删改等不同操作',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
...
exports.cardkitV1CardCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书卡片-卡片-创建卡片实体-基于卡片 JSON 代码或卡片搭建工具搭建的卡片，创建卡片实体。用于后续通过卡片实体 ID（card_id）发送卡片、更新卡片等',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            type: zod_1.z
                .string()
                .describe('卡片类型。可选值：- `card_json`：由卡片 JSON 代码构建的卡片- `template`：由[卡片搭建工具]搭建的卡片模板'),
            data: zod_1.z
                .string()
                .describe('卡片数据。需要与 `type` 指定的类型一致：- 若 `type` 为 `card_json`，则此处应传卡片 JSON 代码，并确保将其转义为字符串。仅支持[卡片 JSON 2.0 结构]，即你必须声明 `schema` 为 `2.0`- 若 `type` 为 `template`，则此处应传卡片模板的数据，并确保将其转义为字符串。仅支持新版卡片。即在搭建工具中，卡片名称旁应有“新版”标识'),
        }),
    },
};
exports.cardkitV1CardElementContent = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            content: zod_1.z
                .string()
                .describe('新的全量文本内容。使用时请注意转义为字符串。**注意**：- 若 content 中含有代码块，你需将代码块前后的空格去掉，否则可能导致代码渲染失败。- 若旧文本为传入的新文本的前缀子串，新增文本将在旧文本末尾继续以打字机效果输出；若新旧文本前缀不同，全量文本将直接上屏输出，无打字机效果'),
            sequence: zod_1.z
                .number()
                .describe('操作卡片的序号。用于保证多次更新的时序性。**注意**：请确保在通过卡片 OpenAPI 操作同一张卡片时，sequence 的值相较于上一次操作严格递增。**数据校验规则**：int32 范围（ `1`~`2147483647`）内的正整数'),
        }),
        path: zod_1.z.object({
            card_id: zod_1.z.string().describe('卡片实体 ID。通过[创建卡片实体]获取'),
            element_id: zod_1.z
                .string()
                .describe('卡片实体中，普通文本元素或富文本组件的 ID。对应卡片 JSON 中的 `element_id` 属性或搭建工具中的组件 ID 属性，由开发者自定义。**注意**： - 仅支持[卡片 JSON 2.0 结构]或卡片搭建工具搭建的[新版卡片]。- 对于搭建工具中的卡片，此处仅支持传入富文本组件的组件 ID。即仅支持对富文本组件中的内容进行流式更新'),
        }),
    },
};
exports.cardkitV1CardElementCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-飞书卡片-组件-新增组件-为指定卡片实体新增组件，以扩展卡片内容，如在卡片中添加一个点击按钮',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            type: zod_1.z
            ...
            target_element_id: zod_1.z
                .string()
                .describe('目标组件的 ID。 填写规则如下所示：- 当 `type` 为 `insert_before`、`insert_after` 时，字段必填，为用于定位的目标组件- 当 `type` 为 `append` 时，该字段必填且仅支持容器类组件，为用于指定末尾添加的目标组件- 未填写默认为在卡片 body 末尾添加')
                .optional(),
            uuid: zod_1.z.string().describe('幂等 ID，可通过传入唯一的 UUID 以保证相同批次的操作只进行一次').optional(),
            sequence: zod_1.z
                .number()
            ...
            elements: zod_1.z
                .string()
                .describe('添加的组件列表。**注意**：- 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串。- 本参数仅支持[卡片 JSON 2.0 结构]'),
        }),
        path: zod_1.z.object({ card_id: zod_1.z.string().describe('要新增组件的卡片实体 ID。通过[创建卡片实体]获取') }),
    },
};
...
exports.cardkitV1CardElementPatch = {
    ...
    path: '/open-apis/cardkit/v1/cards/:card_id/elements/:element_id',
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-飞书卡片-组件-更新组件属性-通过传入 `card_id`（卡片实体 ID）和 `element_id`（组件 ID），更新卡片实体中对应组件的属性',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            partial_element: zod_1.z
                .string()
                .describe('组件的新的配置项字段。传入 `element_id` 参数后，原组件的 ID 将更新。**注意**：- 不支持修改 `tag` 参数。 - 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串。- 仅支持[卡片 JSON 2.0 结构]'),
            uuid: zod_1.z.string().describe('幂等 ID，可通过传入唯一的 UUID 以保证相同批次的操作只进行一次').optional(),
            sequence: zod_1.z
                .number()
                .describe('操作卡片的序号。用于保证多次更新的时序性。**注意**：请确保在通过卡片 OpenAPI 操作同一张卡片时，sequence 的值相较于上一次操作严格递增。**数据校验规则**：int32 范围（ `1`~`2147483647`）内的正整数'),
...
exports.cardkitV1CardElementUpdate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            element: zod_1.z
                .string()
                .describe('新的组件的完整的 JSON 数据。注意：- 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串。- 仅支持[卡片 JSON 2.0 结构]'),
            sequence: zod_1.z
                .number()
                .describe('操作卡片的序号。用于保证多次更新的时序性。**注意**：请确保在通过卡片 OpenAPI 操作同一张卡片时，sequence 的值相较于上一次操作严格递增。**数据校验规则**：int32 范围（ `1`~`2147483647`）内的正整数'),
        }),
        path: zod_1.z.object({
            ...
            element_id: zod_1.z
                .string()
                .describe('要更新的组件 ID。对应卡片 JSON 中组件的 `element_id` 属性，由开发者自定义。**提示**：同一张卡片内字段值唯一。仅允许使用字母、数字和下划线，必须以字母开头'),
        }),
    },
};
exports.cardkitV1CardIdConvert = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-历史版本（不推荐）-转换 ID-将[消息 ID]（ `message_id` ）转换为卡片实体 ID（`card_id`）。用于将由[发送消息]等接口返回的消息 ID 转换为卡片实体 ID，以进一步对卡片进行全量更新、局部更新、或文本流式更新操作',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            message_id: zod_1.z
...
exports.cardkitV1CardSettings = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-飞书卡片-卡片-更新卡片实体配置-更新指定卡片实体的配置，支持卡片配置 `config` 字段和卡片跳转链接 `card_link` 字段',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            settings: zod_1.z
                .string()
                .describe('卡片配置相关字段转义后的字符串，包括 `config` 和 `card_link` 字段。**注意**：- 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串。- 本字段仅支持[卡片 JSON 2.0 结构]中的对应字段'),
            uuid: zod_1.z.string().describe('幂等 ID，可通过传入唯一的 UUID 以保证相同批次的操作只进行一次').optional(),
            sequence: zod_1.z
                .number()
                .describe('操作卡片的序号。用于保证多次更新的时序性。**注意**：请确保在通过卡片 OpenAPI 操作同一张卡片时，sequence 的值相较于上一次操作严格递增。**数据校验规则**：int32 范围（ `1`~`2147483647`）内的正整数'),
    ...
    schema: {
        data: zod_1.z.object({
            card: zod_1.z
                ...
                data: zod_1.z
                    .string()
                    .describe('卡片 JSON 数据的内容。**注意**：- 仅支持 JSON 2.0 版本的卡片结构。- 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串'),
            })
