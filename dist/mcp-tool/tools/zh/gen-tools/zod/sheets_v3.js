    ...
    schema: {
        data: zod_1.z
            ...
            folder_token: zod_1.z
                .string()
                .describe('文件夹 token。你可通过以下两种方式获取文件夹的 token：- 文件夹的 URL：https://sample.feishu.cn/drive/folder/==fldbcO1UuPz8VwnpPx5a92abcef==- 调用开放平台接口获取： - 调用[获取我的空间（root folder）元数据]接口获取根目录（即根文件夹）的 token。 - 继续调用[获取文件夹中的文件清单]接口，获取根目录下文件夹的 token。**提示**：要在知识库中创建电子表格，你需调用[创建知识空间节点]接口，并选择表格（sheet）类型')
                .optional(),
        })
            .optional(),
...
exports.sheetsV3SpreadsheetSheetFilterViewConditionCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-创建筛选条件-在筛选视图的指定列创建筛选条件，包括筛选的类型、比较类型、筛选参数等',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z
            .object({
            ...
            filter_type: zod_1.z
                .string()
                .describe('筛选类型。枚举值如下所示。了解更多，参考[筛选条件指南]。- hiddenValue：隐藏值筛选- number：数字筛选- text：文本筛选- color：颜色筛选')
                .optional(),
            compare_type: zod_1.z.string().describe('比较类型。了解更多，参考[筛选条件指南]').optional(),
            expected: zod_1.z.array(zod_1.z.string()).describe('筛选参数。了解更多，参考[筛选条件指南]').optional(),
        })
            .optional(),
        path: zod_1.z
            .object({
            spreadsheet_token: zod_1.z
                ...
                .describe('电子表格的 token。可通过以下两种方式获取。了解更多，参考[电子表格概述]。- 电子表格的 URL：https://sample.feishu.cn/sheets/==Iow7sNNEphp3WbtnbCscPqabcef==- 调用[获取文件夹中的文件清单]')
                .optional(),
            sheet_id: zod_1.z.string().describe('工作表 ID，通过[获取工作表] 获取').optional(),
            filter_view_id: zod_1.z.string().describe('筛选视图 ID。通过[查询筛选视图]获取').optional(),
        })
            .optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.sheetsV3SpreadsheetSheetFilterViewConditionDelete = {
    ...
    httpMethod: 'DELETE',
    description: '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-删除筛选条件-删除筛选视图指定列的所有筛选条件',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z
            .object({
...
exports.sheetsV3SpreadsheetSheetFilterViewConditionGet = {
    ...
    path: '/open-apis/sheets/v3/spreadsheets/:spreadsheet_token/sheets/:sheet_id/filter_views/:filter_view_id/conditions/:condition_id',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-获取筛选条件-获取筛选视图某列的筛选条件，包括筛选的类型、比较类型、筛选参数等',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z
            .object({
...
exports.sheetsV3SpreadsheetSheetFilterViewConditionQuery = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-查询筛选条件-查询指定筛选视图的所有筛选条件，包括筛选的类型、比较类型、筛选参数等',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z
            .object({
...
exports.sheetsV3SpreadsheetSheetFilterViewConditionUpdate = {
    ...
    path: '/open-apis/sheets/v3/spreadsheets/:spreadsheet_token/sheets/:sheet_id/filter_views/:filter_view_id/conditions/:condition_id',
    httpMethod: 'PUT',
    description: '[Feishu/Lark]-云文档-电子表格-筛选视图-筛选条件-更新筛选条件-更新筛选视图指定列的筛选条件，包括筛选的类型、比较类型、筛选参数等',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z
            .object({
...
exports.sheetsV3SpreadsheetSheetFilterViewCreate = {
    ...
    schema: {
        data: zod_1.z
            ...
            range: zod_1.z
                .string()
                .describe('筛选视图的筛选范围。该参数必填，请忽略必填列的“否”。支持以下五种写法，了解更多，参考[筛选指南]。- `sheetId`：填写实际的工作表 ID，表示将筛选应用于整表- `sheetId!{开始行索引}:{结束行索引}`：填写工作表 ID 和行数区间，表示将筛选应用于整行- `sheetId!{开始列索引}:{结束列索引}`：填写工作表 ID 和列的区间，表示将筛选应用于整列- `sheetId!{开始单元格}:{结束单元格}`：填写工作表 ID 和单元格区间，表示将筛选应用于单元格选定的区域中- `sheetId!{开始单元格}:{结束列索引}`：填写工作表 ID、起始单元格和结束列，表示省略结束行，使用表格的最后行作为结束行')
                .optional(),
        })
            .optional(),
        path: zod_1.z
...
exports.sheetsV3SpreadsheetSheetFilterViewPatch = {
    ...
    schema: {
        data: zod_1.z
            ...
            range: zod_1.z
                .string()
                .describe('筛选视图的筛选范围。支持以下五种写法，了解更多，参考[筛选指南]。- sheetId：填写实际的工作表 ID，表示将筛选应用于整表- sheetId!1:2 ：填写工作表 ID 和行数区间，表示将筛选应用于整行- sheetId!A:B ：填写工作表 ID 和列的区间，表示将筛选应用于整列- sheetId!A1:B2 ：填写工作表 ID 和单元格区间，表示将筛选应用于单元格选定的区域中- sheetId!A1:C ：填写工作表 ID、起始单元格和结束列，表示省略结束行，使用表格的最后行作为结束行')
                .optional(),
        })
            .optional(),
        path: zod_1.z
...
exports.sheetsV3SpreadsheetSheetFilterCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            range: zod_1.z
                .string()
                .describe('设置筛选的应用范围。支持以下五种写法，了解更多，参考[筛选指南]。- `sheetId`：填写实际的工作表 ID，表示将筛选应用于整表- `sheetId!{开始行索引}:{结束行索引}` ：填写工作表 ID 和行数区间，表示将筛选应用于整行- `sheetId!{开始列索引}:{结束列索引}`：填写工作表 ID 和列的区间，表示将筛选应用于整列- `sheetId!{开始单元格}:{结束单元格}`：填写工作表 ID 和单元格区间，表示将筛选应用于单元格选定的区域中- `sheetId!{开始单元格}:{结束列索引}`：填写工作表 ID、起始单元格和结束列，表示省略结束行，使用表格的最后行作为结束行'),
            col: zod_1.z.string().describe('设置应用筛选条件的列'),
            condition: zod_1.z
                .object({
                filter_type: zod_1.z
                    .string()
                    .describe('筛选类型，枚举值如下所示。了解更多，参考[筛选指南]。- multiValue ：多值筛选- number ：数字筛选- text ：文本筛选- color ：颜色筛选- clear ：清除某列的筛选条件'),
        ...
        data: zod_1.z.object({
            ...
            condition: zod_1.z
                ...
                filter_type: zod_1.z
                    .string()
                    .describe('筛选类型，枚举值如下所示。了解更多，参考[筛选指南]。- multiValue ：多值筛选- number ：数字筛选- text ：文本筛选- color ：颜色筛选- clear ：清除某列的筛选条件'),
                compare_type: zod_1.z
                    .string()
                    .describe('比较类型。不同筛选类型的比较类型的枚举值不同，详情参考[筛选指南]')
                    .optional(),
                expected: zod_1.z.array(zod_1.z.string()).describe('筛选参数。不同筛选类型的筛选参数限制不同，详情参考[筛选指南]'),
            })
                .describe('设置筛选条件'),
        }),
        path: zod_1.z
    ...
    schema: {
        data: zod_1.z.object({
            find_condition: zod_1.z
                ...
                range: zod_1.z
                    .string()
                    .describe('查找范围。格式为 `<sheetId>!<开始位置>:<结束位置>`。其中：- `sheetId` 为工作表 ID，通过[获取工作表] 获取- `<开始位置>:<结束位置>` 为工作表中单元格的范围，数字表示行索引，字母表示列索引。如 `A2:B2` 表示该工作表第 2 行的 A 列到 B 列。`range`支持四种写法，详情参考[电子表格概述]'),
                match_case: zod_1.z
                    .boolean()
    ...
    schema: {
        data: zod_1.z
            ...
            float_image_token: zod_1.z
                .string()
                .describe('浮动图片的 token。通过[上传素材]或[分片上传素材]上传图片至表格，获得素材的 `file_token`，即为 float_image_token。**注意**：该参数必填，请忽略左侧必填列的”否”')
                .optional(),
            range: zod_1.z
                .string()
                .describe('浮动图片左上角所在单元格位置，只允许单个单元格的形式，如 "ahgsch!A1:A1"。了解更多，参考[浮动图片使用指南]。**注意**：该参数必填，请忽略左侧必填列的”否”')
                .optional(),
            width: zod_1.z
                .number()
                .describe('浮动图片的宽度，单位为像素。不传会默认采用图片实际宽度，如果传则需要大于等于 20 像素。了解更多，参考[浮动图片使用指南]')
                .optional(),
            height: zod_1.z
                .number()
                .describe('浮动图片的高度，单位为像素。不传会默认采用图片实际高度，如果传则需要大于等于 20 像素。了解更多，参考[浮动图片使用指南]')
                .optional(),
            offset_x: zod_1.z
                .number()
                .describe('浮动图片左上角距离所在单元格左上角的横向偏移，单位为像素，默认为 0，设置的值需要大于等于 0、小于浮动图片左上角所在单元格的宽度。了解更多，参考[浮动图片使用指南]')
                .optional(),
            offset_y: zod_1.z
                .number()
                .describe('浮动图片左上角距离所在单元格左上角的纵向偏移，单位为像素，默认为 0。设置的值需要大于等于 0、小于浮动图片左上角所在单元格的高度。了解更多，参考[浮动图片使用指南]')
                .optional(),
        })
            .optional(),
        path: zod_1.z
            ...
            spreadsheet_token: zod_1.z
                ...
                .describe('电子表格的 token。可通过以下两种方式获取。了解更多，参考[电子表格概述]。- 电子表格的 URL：https://sample.feishu.cn/sheets/==Iow7sNNEphp3WbtnbCscPqabcef==- 调用[获取文件夹中的文件清单]')
                .optional(),
            sheet_id: zod_1.z.string().describe('电子表格工作表的 ID。调用[获取工作表]获取 ID').optional(),
        })
            .optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
    ...
    schema: {
        path: zod_1.z
            ...
            spreadsheet_token: zod_1.z
                ...
                .optional(),
            sheet_id: zod_1.z.string().describe('电子表格工作表的 ID。调用[获取工作表]获取 ID').optional(),
            float_image_id: zod_1.z.string().describe('工作表内浮动图片的唯一标识。通过[查询浮动图片]接口获取').optional(),
        })
            .optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.sheetsV3SpreadsheetSheetFloatImagePatch = {
    ...
    schema: {
        data: zod_1.z
            ...
            range: zod_1.z
                .string()
                .describe('浮动图片左上角所在单元格位置，只允许单个单元格的形式，如 "ahgsch!A1:A1"。了解更多，参考[浮动图片使用指南]')
                .optional(),
            width: zod_1.z
                .number()
                .describe('浮动图片的宽度，单位为像素。不传会默认采用图片实际宽度，如果传则需要大于等于 20 像素。了解更多，参考[浮动图片使用指南]')
    ...
    schema: {
        path: zod_1.z.object({
            spreadsheet_token: zod_1.z
                .string()
                .describe('电子表格的 token。可通过以下两种方式获取。了解更多，参考[电子表格概述]。- 电子表格的 URL：https://sample.feishu.cn/sheets/==Iow7sNNEphp3WbtnbCscPqabcef==- 调用[获取文件夹中的文件清单]'),
            sheet_id: zod_1.z.string().describe('工作表的 ID。调用[获取工作表]获取 ID'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
    ...
    schema: {
        ...
        path: zod_1.z
            ...
            spreadsheet_token: zod_1.z
                ...
                .describe('电子表格的 token。可通过以下两种方式获取。了解更多，参考[电子表格概述]。- 电子表格的 URL：https://sample.feishu.cn/sheets/==Iow7sNNEphp3WbtnbCscPqabcef==- 调用[获取文件夹中的文件清单]')
                .optional(),
            sheet_id: zod_1.z.string().describe('工作表的 ID。调用[获取工作表]获取 ID').optional(),
        })
            .optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
