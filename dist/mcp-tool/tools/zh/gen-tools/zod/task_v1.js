    ...
    schema: {
        data: zod_1.z
            ...
            id_list: zod_1.z
                ...
                .describe('执行者的用户ID列表。传入的值为 user_id 或 open_id，由user_id_type 决定。user_id和open_id的获取可见文档[如何获取不同的用户 ID]')
                .optional(),
        })
            .optional(),
        params: zod_1.z
...
exports.taskV1TaskBatchDeleteFollower = {
    ...
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({ id_list: zod_1.z.array(zod_1.z.string()).describe('要删除的关注人ID列表').optional() }).optional(),
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() })
            .optional(),
        path: zod_1.z.object({ task_id: zod_1.z.string().describe('任务ID') }),
...
exports.taskV1TaskCollaboratorCreate = {
    ...
    schema: {
        data: zod_1.z
            ...
            id: zod_1.z
                .string()
                .describe('任务执行者的 ID。传入的值为 user_id 或 open_id，由user_id_type 决定。user_id和open_id的获取可见文档[如何获取不同的用户 ID]。<md-alert>已经废弃，为了向前兼容早期只支持单次添加一个人的情况而保留，但不再推荐使用，建议使用id_list字段</md-alert>')
                .optional(),
            id_list: zod_1.z
                .array(zod_1.z.string())
                .describe('执行者的用户ID列表。传入的值为 user_id 或 open_id，由user_id_type 决定。user_id和open_id的获取可见文档[如何获取不同的用户 ID]')
        ...
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() })
            .optional(),
        path: zod_1.z.object({ task_id: zod_1.z.string().describe('任务 ID，可通过[创建任务]时响应体中的id字段获取') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.taskV1TaskCollaboratorList = {
    ...
    schema: {
        params: zod_1.z
            ...
            .optional(),
        path: zod_1.z.object({ task_id: zod_1.z.string().describe('任务 ID').optional() }).optional(),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.taskV1TaskCommentCreate = {
    ...
    schema: {
        data: zod_1.z
            ...
            rich_content: zod_1.z.string().describe('富文本评论内容。语法格式参见[Markdown模块]').optional(),
        })
            .optional(),
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() })
        ...
        data: zod_1.z.object({
            ...
            origin: zod_1.z
                ...
                platform_i18n_name: zod_1.z
                    .string()
                    .describe('任务来源的名称。用于在任务中心详情页展示。需要提供一个字典，支持多种语言名称映射。应用在使用不同语言时，导入来源也将展示对应的内容。详细参见：[任务字段补充说明]'),
                href: zod_1.z
                    .object({
                    url: zod_1.z
                        .string()
            ...
            collaborator_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('创建任务时添加的执行者用户id列表。传入的值为 user_id 或 open_id ，由user_id_type 决定。user_id和open_id的获取可见文档：[如何获取不同的用户 ID]')
                .optional(),
            follower_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('创建任务时添加的关注者用户id列表。传入的值为 user_id 或 open_id ，由user_id_type 决定。user_id和open_id的获取可见文档：[如何获取不同的用户 ID]')
                .optional(),
            repeat_rule: zod_1.z.string().describe('重复任务的规则表达式。语法格式参见[RRule语法规范] 4.3.10小节').optional(),
            rich_summary: zod_1.z
                .string()
                .describe('富文本任务标题。语法格式参见[Markdown模块]。创建任务时，任务标题(summary字段)和任务富文本标题(rich_summary字段)不能同时为空，需要至少填充其中一个字段')
                .optional(),
            rich_description: zod_1.z.string().describe('富文本任务备注。语法格式参见[Markdown模块]').optional(),
        }),
        params: zod_1.z
            .object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() })
            .optional(),
